...
public class DictTypeServiceImpl implements DictTypeService {
    ...
    public void deleteDictType(Long id) {
        ...
        dictDataService.deleteByDictType(List.of( dictType.getType()));
        // 删除字典类型
    ...
    public void deleteDictTypeList(List<Long> ids) {
        ...
        List<String> list = dictTypes.stream().map(p -> p.getType()).toList();
        if(ObjectUtil.isNotEmpty(list)) {
            dictDataService.deleteByDictType(list);
        }

        // 2. 批量删除字典类型
        ids.forEach(id -> dictTypeMapper.updateToDelete(id));
    }
