<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" >
      <el-form-item label="调度单号" prop="no">
        <el-input v-model="queryParams.no" placeholder="请输入调度单号" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      ...
      <el-form-item label="调度时间" prop="moveTime">
        <el-date-picker v-model="queryParams.moveTime" value-format="YYYY-MM-DD HH:mm:ss" type="daterange"
          start-placeholder="开始日期" end-placeholder="结束日期"
          :default-time="[new Date('1 00:00:00'), new Date('1 23:59:59')]" class="!w-240px" />
      </el-form-item>
      <el-form-item label="仓库" prop="fromWarehouseId">
        <el-select v-model="queryParams.fromWarehouseId" clearable filterable placeholder="请选择仓库" class="!w-240px">
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['erp:stock-move:create']">
        ...
        <el-button type="danger" plain @click="handleDelete(selectionList.map((item) => item.id))"
          v-hasPermi="['erp:stock-move:delete']" :disabled="selectionList.length === 0">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true"
      ...
      <el-table-column label="调度时间" align="center" prop="moveTime" :formatter="dateFormatter2" width="120px" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="220">
        <template #default="scope">
          <el-button link @click="openForm('detail', scope.row.id)" v-hasPermi="['erp:stock-move:query']">
            详情
          </el-button>
          <el-button link type="primary" @click="openForm('update', scope.row.id)"
            v-hasPermi="['erp:stock-move:update']" :disabled="scope.row.status === 20">
            编辑
          </el-button>
          <el-button link type="primary" @click="handleUpdateStatus(scope.row.id, 20)"
            v-hasPermi="['erp:stock-move:update-status']" v-if="scope.row.status === 10">
            审批
          </el-button>
          <el-button link type="danger" @click="handleUpdateStatus(scope.row.id, 10)"
            v-hasPermi="['erp:stock-move:update-status']" v-else>
            反审批
          </el-button>
          <el-button link type="danger" @click="handleDelete([scope.row.id])" v-hasPermi="['erp:stock-move:delete']">
            删除
          </el-button>
        </template>
      </el-table-column>
