<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" >
      <el-form-item label="盘点单号" prop="no">
        <el-input v-model="queryParams.no" placeholder="请输入盘点单号" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      ...
      <el-form-item label="盘点时间" prop="checkTime">
        <el-date-picker v-model="queryParams.checkTime" value-format="YYYY-MM-DD HH:mm:ss" type="daterange"
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['erp:stock-check:create']">
        ...
        <el-button type="danger" plain @click="handleDelete(selectionList.map((item) => item.id))"
          v-hasPermi="['erp:stock-check:delete']" :disabled="selectionList.length === 0">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true"
      ...
      <el-table-column label="盘点时间" align="center" prop="checkTime" :formatter="dateFormatter2" width="120px" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="220">
        <template #default="scope">
          <el-button link @click="openForm('detail', scope.row.id)" v-hasPermi="['erp:stock-check:query']">
            详情
          </el-button>
          <el-button link type="primary" @click="openForm('update', scope.row.id)"
            v-hasPermi="['erp:stock-check:update']" :disabled="scope.row.status === 20">
            编辑
          </el-button>
          <el-button link type="primary" @click="handleUpdateStatus(scope.row.id, 20)"
            v-hasPermi="['erp:stock-check:update-status']" v-if="scope.row.status === 10">
            审批
          </el-button>
          <el-button link type="danger" @click="handleUpdateStatus(scope.row.id, 10)"
            v-hasPermi="['erp:stock-check:update-status']" v-else>
            反审批
          </el-button>
          <el-button link type="danger" @click="handleDelete([scope.row.id])" v-hasPermi="['erp:stock-check:delete']">
            删除
          </el-button>
        </template>
      </el-table-column>
