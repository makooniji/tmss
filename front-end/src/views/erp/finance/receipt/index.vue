<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" >
      <el-form-item label="收款单号" prop="no">
        <el-input v-model="queryParams.no" placeholder="请输入收款单号" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      <el-form-item label="收款时间" prop="receiptTime">
        <el-date-picker v-model="queryParams.receiptTime" value-format="YYYY-MM-DD HH:mm:ss" type="daterange"
      ...
      <el-form-item label="收款账户" prop="accountId">
        <el-select v-model="queryParams.accountId" clearable filterable placeholder="请选择收款账户" class="!w-240px">
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['erp:finance-receipt:create']">
        ...
        <el-button type="danger" plain @click="handleDelete(selectionList.map((item) => item.id))"
          v-hasPermi="['erp:finance-receipt:delete']" :disabled="selectionList.length === 0">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true"
      ...
      <el-table-column label="收款时间" align="center" prop="receiptTime" :formatter="dateFormatter2" width="120px" />
      ...
      <el-table-column label="合计收款" align="center" prop="totalPrice" :formatter="erpPriceTableColumnFormatter" />
      <el-table-column label="优惠金额" align="center" prop="discountPrice" :formatter="erpPriceTableColumnFormatter" />
      <el-table-column label="实际收款" align="center" prop="receiptPrice" :formatter="erpPriceTableColumnFormatter" />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="220">
        <template #default="scope">
          <el-button link @click="openForm('detail', scope.row.id)" v-hasPermi="['erp:finance-receipt:query']">
            详情
          </el-button>
          <el-button link type="primary" @click="openForm('update', scope.row.id)"
            v-hasPermi="['erp:finance-receipt:update']" :disabled="scope.row.status === 20">
            编辑
          </el-button>
          <el-button link type="primary" @click="handleUpdateStatus(scope.row.id, 20)"
            v-hasPermi="['erp:finance-receipt:update-status']" v-if="scope.row.status === 10">
            审批
          </el-button>
          <el-button link type="danger" @click="handleUpdateStatus(scope.row.id, 10)"
            v-hasPermi="['erp:finance-receipt:update-status']" v-else>
            反审批
          </el-button>
