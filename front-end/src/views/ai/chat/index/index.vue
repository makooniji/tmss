  ...
  <el-container class="absolute flex-1 top-0 left-0 h-full w-full">
    ...
    <el-container class="bg-[var(--el-bg-color)]">
        ...
        <div class="text-18px font-bold">
          {{
            activeConversation?.title
              ? activeConversation?.title
              : t('aiChat.index.defaultTitle')
          }}
          <span v-if="activeMessageList.length"
            >({{ activeMessageList.length }})</span
          >
        </div>
        <div
          class="flex w-300px flex-row justify-end"
      ...
      <el-main class="m-0 p-0 relative h-full w-full">
        <div>
          <div
            class="absolute top-0 bottom-0 left-0 right-0 overflow-y-hidden p-0 m-0"
          >
            <!-- 情况一：消息加载中 -->
      ...
      <el-footer class="flex flex-col !h-auto !p-0">
        ...
        >
          <textarea
            ...
            :placeholder="t('aiChat.index.inputPlaceholder')"
          >
          </textarea>
          <div class="flex justify-between pb-0 pt-5px">
            <div class="flex items-center">
              ...
              <span class="ml-5px mr-15px text-14px text-#8f8f8f">{{
                t('aiChat.index.context')
              }}</span>
              <el-switch v-model="enableWebSearch" />
              <span class="ml-5px text-14px text-#8f8f8f">{{
                t('aiChat.index.webSearch')
              }}</span>
            </div>
            <el-button
              type="primary"
...
<script setup lang="ts">
  ...
  const getConversation = async (id: number | null) => {
    ...
    const conversation: ChatConversationVO =
      await ChatConversationApi.getChatConversationMy(id)
    if (!conversation) {
      return
    }
  ...
  const handleConversationClick = async (conversation: ChatConversationVO) => {
    ...
    if (conversationInProgress.value) {
      message.alert(t('aiChat.index.noSwitchWhileChat'))
      return false
    }
  ...
  const handlerConversationDelete = async (
    delConversation: ChatConversationVO
  ) => {
    // 删除的对话如果是当前选中的，那么就重置
  ...
  const getMessageList = async () => {
    try {
      ...
      // 获取消息列表
      activeMessageList.value =
        await ChatMessageApi.getChatMessageListByConversationId(
  ...
  const handleMessageDelete = () => {
    if (conversationInProgress.value) {
      message.alert(t('aiChat.index.noDeleteWhileAnswering'))
      return
    }
    // 刷新 message 列表
    getMessageList()
  ...
  const handlerMessageClear = async () => {
    ...
    try {
      // 确认提示
      await message.delConfirm(t('aiChat.index.confirmClearMessages'))
      // 清空对话
  ...
  const doSendMessage = async (content: string) => {
    // 校验
    if (content.length < 1) {
      message.error(t('aiChat.index.errEmpty'))
      return
    }
    if (activeConversationId.value == null) {
      message.error(t('aiChat.index.errNoConversation'))
      return
    }
  ...
  const doSendMessageStream = async (userMessage: ChatMessageVO) => {
    ...
    try {
      ...
      activeMessageList.value.push({
        ...
        conversationId: activeConversationId.value,
        type: 'assistant',
        content: t('aiChat.index.thinking'),
        reasoningContent: '',
        createTime: new Date()
      } as ChatMessageVO)
      // 1.2 滚动到最下面
      ...
      await ChatMessageApi.sendChatMessageStream(
        ...
        async (res) => {
          ...
          if (code !== 0) {
            message.alert(t('aiChat.index.errStreamWithMsg', { msg }))
            // 如果未接收到消息，则进行删除
        ...
        (error: any) => {
          // 异常提示，并停止流
          message.alert(t('aiChat.index.errStream'))
          stopStream()
          // 需要抛出异常，禁止重试
          throw error
        },
