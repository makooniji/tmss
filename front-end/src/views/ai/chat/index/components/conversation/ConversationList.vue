  ...
  >
    ...
    <div class="h-100%">
      ...
      >
        ...
        {{ t('aiChat.conversationList.newChat') }}
      </el-button>
      ...
      <el-input
        ...
        size="large"
        class="mt-5"
        :placeholder="t('aiChat.conversationList.searchHistory')"
      ...
      <div class="overflow-auto h-full">
        ...
        <div v-for="groupKey in CONV_GROUP_KEYS" :key="groupKey">
          <div class="mt-1.25 pt-2.5" v-if="conversationMap[groupKey]?.length">
            <el-text class="mx-1" size="small" tag="b">
              {{ t(`aiChat.convGroup.${groupKey}`) }}
            </el-text>
          </div>
          <div
            class="mt-1.25"
            v-for="conversation in conversationMap[groupKey] || []"
            :key="conversation.id"
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <el-icon
                    :title="t('aiChat.conversationList.pin')"
                ...
                >
                  <el-icon :title="t('aiChat.conversationList.edit')">
                    <Icon icon="ep:edit" />
                  </el-icon>
                </el-button>
                <el-button
                ...
                >
                  <el-icon :title="t('aiChat.conversationList.deleteConv')">
                    <Icon icon="ep:delete" />
                  </el-icon>
                </el-button>
              </div>
    ...
    >
      ...
      >
        <Icon icon="ep:user" />
        <el-text class="ml-1.25" size="small">{{
          t('aiChat.conversationList.roleRepo')
        }}</el-text>
      </div>
      <div
        class="flex items-center p-0 m-0 cursor-pointer"
      ...
      >
        <Icon icon="ep:delete" />
        <el-text class="ml-1.25" size="small">{{
          t('aiChat.conversationList.clearUnpinned')
        }}</el-text>
      </div>
    </div>
    ...
    <el-drawer
      ...
      :title="t('aiChat.conversationList.drawerTitle')"
      size="754px"
    >
      <RoleRepository />
    </el-drawer>
...
<script setup lang="ts">
  ...
  /** 侧边栏时间分组顺序（key 对应 aiChat.convGroup.*） */
  const CONV_GROUP_KEYS = [
    'pinned',
    'today',
    'oneDayAgo',
    'threeDaysAgo',
  ...
  const getChatConversationList = async () => {
    try {
      ...
      // 1.1 获取 对话数据
      conversationList.value =
        await ChatConversationApi.getChatConversationMyList()
      // 1.2 排序
  ...
  const getConversationGroupByCreateTime = async (
    list: ChatConversationVO[]
  ) => {
    const groupMap: Record<
      (typeof CONV_GROUP_KEYS)[number],
      ChatConversationVO[]
    > = {
      pinned: [],
      today: [],
  ...
  const updateConversationTitle = async (conversation: ChatConversationVO) => {
    ...
    const { value } = await ElMessageBox.prompt(
      t('aiChat.conversationList.renameTitle'),
      {
        inputPattern: /^[\s\S]*.*\S[\s\S]*$/, // 判断非空，且非空格
        inputErrorMessage: t('aiChat.conversationList.titleRequired'),
    ...
    message.success(t('aiChat.conversationList.renameOk'))
    // 3. 刷新列表
    await getChatConversationList()
    // 4. 过滤当前切换的
  ...
  const deleteChatConversation = async (conversation: ChatConversationVO) => {
    try {
      ...
      await message.delConfirm(
        t('aiChat.conversationList.deleteConfirm', {
          title: conversation.title
        })
      )
      // 发起删除
      await ChatConversationApi.deleteChatConversationMy(conversation.id)
      message.success(t('aiChat.conversationList.deleted'))
      // 刷新列表
      await getChatConversationList()
      // 回调
  ...
  const handleClearConversation = async () => {
    try {
      await message.confirm(t('aiChat.conversationList.clearConfirm'))
      await ChatConversationApi.deleteChatConversationMyByUnpinned()
      ElMessage({
        message: t('aiChat.conversationList.opOk'),
        type: 'success'
      })
      // 清空 对话 和 对话内容
      activeConversationId.value = null
