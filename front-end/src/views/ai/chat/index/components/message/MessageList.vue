<template>
  <div ref="messageContainer" class="h-100% overflow-y-auto relative">
    <div
      class="flex flex-col overflow-y-hidden px-20px"
    ...
    >
      ...
      <!-- 靠右 message：user 类型 -->
      <div
        class="flex flex-row-reverse justify-start mt-50px"
        v-if="item.type === 'user'"
      >
        <div class="avatar">
  ...
  <div
    v-if="isScrolling"
    class="absolute z-1000 bottom-0 right-50%"
...
<script setup lang="ts">
  ...
  const userAvatar = computed(
    () => userStore.user.avatar || userAvatarDefaultImg
  )
  ...
  const scrollToBottom = async (isIgnore?: boolean) => {
    ...
    if (isIgnore || !isScrolling.value) {
      messageContainer.value.scrollTop =
        ...
        messageContainer.value.offsetHeight
    }
  }
  ...
  const copyContent = async (content: string) => {
    await copy(content)
    message.success(t('aiChat.messageList.copySuccess'))
  }
  ...
  const onDelete = async (id) => {
    ...
    message.success(t('aiChat.messageList.deleteSuccess'))
    // 回调
