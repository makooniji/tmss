        ...
        >
          <div class="flex items-center flex-1 min-w-0">
            <Icon
              ...
              class="text-blue-500 mr-2 flex-shrink-0"
            />
            <span
...
<script setup lang="ts">
  ...
  const handleFileSelect = (event: Event) => {
    ...
    if (files.length + fileList.value.length > props.limit) {
      message.error(t('aiChat.fileUpload.errMaxFiles', { limit: props.limit }))
      target.value = '' // 清空输入
      return
    }
    // 处理每个文件
    files.forEach((file) => {
      if (file.size > props.maxSize * 1024 * 1024) {
        message.error(
          t('aiChat.fileUpload.errFileTooLarge', {
            name: file.name,
  ...
  const uploadFile = async (fileItem: FileItem) => {
    try {
      ...
      fileItem.uploading = false
      message.error(
        t('aiChat.fileUpload.errUploadFailed', { name: fileItem.name })
      )
