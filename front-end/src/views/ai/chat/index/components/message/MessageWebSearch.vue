  ...
  >
    ...
    >
      <div class="flex items-center">
        ...
        {{
          t('aiChat.webSearch.resultsCount', { count: webSearchPages.length })
        }}
      </div>
      <Icon
    ...
    <div
      v-show="isExpanded"
      class="flex flex-col gap-8px transition-all duration-200 ease-in-out"
    >
      <div
        ...
        <div class="flex items-start gap-8px">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-4px mb-4px">
              <span class="text-12px text-[#999] truncate">{{
                result.name
              }}</span>
            </div>
            ...
            <!-- 主标题 -->
            <div
              class="text-14px text-[#1a73e8] font-medium mb-4px line-clamp-2 leading-[1.4]"
            >
              {{ result.title }}
            </div>
            ...
            <!-- 描述 -->
            <div
              class="text-13px text-[#666] line-clamp-2 leading-[1.4] mb-4px"
            >
              {{ result.snippet }}
            </div>
      ...
      <div v-if="selectedResult">
        ...
        <div class="flex items-start gap-8px mb-12px">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="text-12px text-[#006621] break-all">{{
              selectedResult.url
            }}</div>
          </div>
        </div>
        ...
        <div class="max-h-[60vh] overflow-y-auto">
          <!-- 简短描述 -->
          <div class="mb-12px">
            <div class="text-14px font-medium text-[#333] mb-6px">{{
              t('aiChat.webSearch.shortDesc')
            }}</div>
            <div
              class="text-14px leading-[1.6] text-[#666] bg-[#f8f9fa] p-10px rounded-6px"
            >
        ...
        <!-- 操作按钮 -->
        <div
          class="flex justify-end gap-8px mt-12px pt-12px border-t border-[#eee]"
        >
          <el-button size="small" @click="dialogVisible = false">{{
            t('aiChat.webSearch.close')
          }}</el-button>
          <el-button
            type="primary"
            size="small"
            @click="openUrl(selectedResult.url)"
          >
            {{ t('aiChat.webSearch.visitOriginal') }}
          </el-button>
        </div>
      </div>
    </template>
