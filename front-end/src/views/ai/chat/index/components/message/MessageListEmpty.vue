...
<template>
  <div class="relative flex flex-row justify-center w-full h-full">
    ...
    <div class="flex flex-col justify-center">
      <div class="text-28px font-bold text-center">{{
        t('aiChat.empty.brand')
      }}</div>
      <div
        class="flex flex-row flex-wrap items-center justify-center w-460px mt-20px"
      >
        <div
...
<script setup lang="ts">
  ...
  const promptList = computed(() => [
    { prompt: t('aiChat.empty.prompt1') },
    { prompt: t('aiChat.empty.prompt2') }
  ])
