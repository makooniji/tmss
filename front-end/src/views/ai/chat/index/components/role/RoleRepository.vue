    ...
    <el-main class="flex-1 overflow-hidden m-0 !p-0 relative">
      <div class="mx-3 mt-3 mb-0 absolute right-0 -top-1.25 z-100">
        ...
        <el-input
          ...
          class="!w-60"
          size="default"
          :placeholder="t('aiChat.role.searchPlaceholder')"
          :suffix-icon="Search"
      ...
      >
        ...
        <el-tab-pane
          :label="t('aiChat.role.publicRoles')"
          name="public-role"
          class="!pt-2"
        >
          <RoleCategoryList
...
<script setup lang="ts">
  ...
  const handlerAddRole = async () => {
    formRef.value.open('my-create', null, t('aiChat.role.addRoleTitle'))
  }
  /** 编辑角色 */
  const handlerCardEdit = async (role) => {
    formRef.value.open('my-update', role.id, t('aiChat.role.editRoleTitle'))
  }
  ...
  const handlerCardUse = async (role) => {
    ...
    const conversationId =
      await ChatConversationApi.createChatConversationMy(data)
