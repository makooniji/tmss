  ...
  >
    <div v-for="role in roleList" :key="role.id">
      ...
      >
          ...
          <el-dropdown @command="handleMoreClick">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                >
                  <Icon icon="ep:delete" color="var(--el-color-danger)" />{{
                    t('aiChat.role.delete')
                  }}
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
        ...
        <div>
          <img
            class="w-40px h-40px rounded-10px overflow-hidden"
            :src="role.avatar"
          />
        </div>
        <div class="ml-10px w-full">
          ...
          <div class="flex flex-row-reverse mt-2px">
            <el-button
              type="primary"
              size="small"
              @click="handleUseClick(role)"
              >{{ t('aiChat.role.use') }}</el-button
            >
          </div>
        </div>
      </el-card>
