      ...
      <ContentWrap>
        ...
        >
          ...
          <el-form-item>
            ...
            >
              <Icon icon="ep:delete" />{{ t('common.batchDelete') }}
            </el-button>
          </el-form-item>
        </el-form>
      </ContentWrap>
    ...
    try {
      ...
      const text =
        row.status === CommonStatusEnum.ENABLE
          ? t('system.user.statusEnable')
          : t('system.user.statusDisable')
  ...
  const handleResetPwd = async (row: UserApi.UserVO) => {
    try {
      ...
      const result = await message.prompt(
        t('system.user.promptNewPassword', { username: row.username }),
        t('common.reminder')
      )
      const password = result.value
      // 发起重置
      await UserApi.resetUserPassword(row.id, password)
      message.success(t('system.user.resetPwdSuccess', { password }))
    } catch {}
  }
