  ...
  <ContentWrap>
    <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="-mb-15px" >
      <el-form-item label="应用名" prop="name">
        <el-input v-model="queryParams.name" class="!w-240px" clearable placeholder="请输入应用名"
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="支付宝配置">
        <el-table-column v-for="channel in alipayChannels" :key="channel.code" :label="channel.name.replace('支付宝', '')"
          align="center">
          <template #default="scope">
            <el-button v-if="isChannelExists(scope.row.channelCodes, channel.code)" circle size="small" type="success"
      ...
      <el-table-column align="center" label="微信配置">
        <el-table-column v-for="channel in wxChannels" :key="channel.code" :label="channel.name.replace('微信', '')"
          align="center">
          <template #default="scope">
            <el-button v-if="isChannelExists(scope.row.channelCodes, channel.code)" circle size="small" type="success"
              @click="openChannelForm(scope.row, channel.code)">
      ...
      <el-table-column align="center" label="钱包支付配置">
        <el-table-column :label="PayChannelEnum.WALLET.name" align="center">
          <template #default="scope">
            <el-button v-if="isChannelExists(scope.row.channelCodes, PayChannelEnum.WALLET.code)" circle size="small"
              type="success" @click="openChannelForm(scope.row, PayChannelEnum.WALLET.code)">
              <Icon icon="ep:check" />
            </el-button>
            <el-button v-else circle size="small" type="danger"
      ...
      <el-table-column align="center" label="模拟支付配置">
        <el-table-column :label="PayChannelEnum.MOCK.name" align="center">
          <template #default="scope">
            <el-button v-if="isChannelExists(scope.row.channelCodes, PayChannelEnum.MOCK.code)" circle size="small"
              type="success" @click="openChannelForm(scope.row, PayChannelEnum.MOCK.code)">
              <Icon icon="ep:check" />
            </el-button>
            <el-button v-else circle size="small" type="danger"
      ...
      <el-table-column align="center" fixed="right" label="操作" min-width="110">
        <template #default="scope">
          <el-button v-hasPermi="['pay:app:update']" link type="primary" @click="openForm('update', scope.row.id)">
            编辑
          </el-button>
          <el-button v-hasPermi="['pay:app:delete']" link type="danger" @click="handleDelete(scope.row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
