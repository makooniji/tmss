<template>
  <div class="flex flex-col">
    <el-row :gutter="16" class="summary">
      <el-col :sm="6" :xs="12">
        <TradeStatisticValue tooltip="昨日订单数量" title="昨日订单数量" :value="summary?.value?.yesterdayOrderCount || 0" :percent="calculateRelativeRate(
      ...
      <el-col :sm="6" :xs="12">
        <TradeStatisticValue tooltip="本月订单数量" title="本月订单数量" :value="summary?.value?.monthOrderCount || 0" :percent="calculateRelativeRate(
      ...
      <el-col :sm="6" :xs="12">
        <TradeStatisticValue tooltip="昨日支付金额" title="昨日支付金额" prefix="￥" :decimals="2"
          :value="fenToYuan(summary?.value?.yesterdayPayPrice || 0)" :percent="calculateRelativeRate(
      ...
      <el-col :sm="6" :xs="12">
        <TradeStatisticValue tooltip="本月支付金额" title="本月支付金额" prefix="￥" ::decimals="2"
          :value="fenToYuan(summary?.value?.monthPayPrice || 0)" :percent="calculateRelativeRate(summary?.value?.monthPayPrice, summary?.reference?.monthPayPrice)
            " />
      </el-col>
    </el-row>
    <el-card shadow="never">
      ...
      <el-row :gutter="16">
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="营业额" tooltip="商品支付金额、充值金额" icon="fa-solid:yen-sign" icon-color="bg-blue-100"
            icon-bg-color="text-blue-500" prefix="￥" :decimals="2"
            :value="fenToYuan(trendSummary?.value?.turnoverPrice || 0)" :percent="calculateRelativeRate(
        ...
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="商品支付金额" tooltip="用户购买商品的实际支付金额，包括微信支付、余额支付、支付宝支付、线下支付金额（拼团商品在成团之后计入，线下支付订单在后台确认支付后计入）"
            icon="fa-solid:shopping-cart" icon-color="bg-purple-100" icon-bg-color="text-purple-500" prefix="￥"
            :decimals="2" :value="fenToYuan(trendSummary?.value?.orderPayPrice || 0)" :percent="calculateRelativeRate(
        ...
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="充值金额" tooltip="用户成功充值的金额" icon="fa-solid:money-check-alt" icon-color="bg-yellow-100"
            icon-bg-color="text-yellow-500" prefix="￥" :decimals="2"
            :value="fenToYuan(trendSummary?.value?.rechargePrice || 0)" :percent="calculateRelativeRate(
        ...
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="支出金额" tooltip="余额支付金额、支付佣金金额、商品退款金额" icon="ep:warning-filled" icon-color="bg-green-100"
            icon-bg-color="text-green-500" prefix="￥" :decimals="2"
            :value="fenToYuan(trendSummary?.value?.expensePrice || 0)" :percent="calculateRelativeRate(
        ...
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="余额支付金额" tooltip="用户下单时使用余额实际支付的金额" icon="fa-solid:wallet" icon-color="bg-cyan-100"
            icon-bg-color="text-cyan-500" prefix="￥" :decimals="2"
            :value="fenToYuan(trendSummary?.value?.walletPayPrice || 0)" :percent="calculateRelativeRate(
        ...
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="支付佣金金额" tooltip="后台给推广员支付的推广佣金，以实际支付为准" icon="fa-solid:award" icon-color="bg-yellow-100"
            icon-bg-color="text-yellow-500" prefix="￥" :decimals="2"
            :value="fenToYuan(trendSummary?.value?.brokerageSettlementPrice || 0)" :percent="calculateRelativeRate(
        ...
        <el-col :md="6" :sm="12" :xs="24">
          <SummaryCard title="商品退款金额" tooltip="用户成功退款的商品金额" icon="fa-solid:times-circle" icon-color="bg-blue-100"
            icon-bg-color="text-blue-500" prefix="￥" :decimals="2"
            :value="fenToYuan(trendSummary?.value?.afterSaleRefundPrice || 0)" :percent="calculateRelativeRate(
