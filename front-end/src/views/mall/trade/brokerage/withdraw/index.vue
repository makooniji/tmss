        ...
        <template #default="scope">
          ...
          <div v-else>
            <div v-if="scope.row.userAccount"
              >账号：{{ scope.row.userAccount }}</div
            >
            <div v-if="scope.row.userName"
              >真实姓名：{{ scope.row.userName }}</div
            >
          </div>
          <template
      ...
      <el-table-column label="操作" align="left" width="110px" fixed="right">
        <template #default="scope">
          <template
            v-if="
              scope.row.status ===
                BrokerageWithdrawStatusEnum.AUDITING.status &&
          ...
          <template
            v-if="
              scope.row.status ===
              BrokerageWithdrawStatusEnum.WITHDRAW_FAIL.status
            "
          >
            <el-button
              link
...
<script setup lang="ts">
  ...
  const getList = async () => {
    ...
    try {
      const data =
        await BrokerageWithdrawApi.getBrokerageWithdrawPage(queryParams)
