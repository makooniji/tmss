  ...
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" >
      ...
      <el-form-item label="用户名称" prop="userNickname">
        <el-input v-model="queryParams.userNickname" placeholder="请输入用户名称" @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      ...
      <el-table-column label="商品信息" align="center" min-width="400">
        <template #default="scope">
          <div class="row flex items-center gap-x-4px">
            <el-image v-if="scope.row.skuPicUrl" :src="scope.row.skuPicUrl" :preview-src-list="[scope.row.skuPicUrl]"
              class="h-40px w-40px shrink-0" preview-teleported />
      ...
      <el-table-column label="评论内容" align="center" prop="content" min-width="210">
        <template #default="scope">
          ...
          <div class="flex justify-center gap-x-4px">
            <el-image v-for="(picUrl, index) in scope.row.picUrls" :key="index" :src="picUrl"
              :preview-src-list="scope.row.picUrls" :initial-index="index" class="h-40px w-40px" preview-teleported />
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否展示" align="center" width="80px">
        <template #default="scope">
          <el-switch v-model="scope.row.visible" :active-value="true" :inactive-value="false"
            v-hasPermi="['product:comment:update']" @change="handleVisibleChange(scope.row)" />
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" min-width="60px" fixed="right">
        <template #default="scope">
          <el-button link type="primary" @click="handleReply(scope.row.id)" v-hasPermi="['product:comment:update']">
            回复
          </el-button>
        </template>
      </el-table-column>
