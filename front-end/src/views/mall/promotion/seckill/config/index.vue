  ...
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="108px">
      <el-form-item label="秒杀时段名称" prop="name">
        <el-input v-model="queryParams.name" placeholder="请输入秒杀时段名称" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['promotion:seckill-config:create']">
    ...
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="秒杀轮播图" align="center" prop="sliderPicUrls">
        <template #default="scope">
          <el-image class="h-40px max-w-40px" v-for="(url, index) in scope?.row.sliderPicUrls" :key="index" :src="url"
            :preview-src-list="scope?.row.sliderPicUrls" :initial-index="index" preview-teleported />
        </template>
      </el-table-column>
