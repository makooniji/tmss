<template>
  ...
  >
    ...
    <el-tabs type="border-card" v-model="activeTabName">
      <el-tab-pane
        :label="t('simpleProcess.childProcess.tabChild')"
        name="child"
      >
        <div>
          <el-form
          ...
          >
            <el-form-item
              :label="t('simpleProcess.childProcess.async')"
              prop="async"
            >
              <el-switch
                v-model="configForm.async"
                :active-text="t('simpleProcess.childProcess.asyncOn')"
                :inactive-text="t('simpleProcess.childProcess.asyncOff')"
              />
            </el-form-item>
            <el-form-item
              :label="t('simpleProcess.childProcess.selectChild')"
            ...
            <el-form-item
              :label="t('simpleProcess.childProcess.skipStartNode')"
              prop="skipStartUserNode"
            >
              <el-switch
                v-model="configForm.skipStartUserNode"
                :active-text="t('simpleProcess.childProcess.skipOn')"
                :inactive-text="t('simpleProcess.childProcess.skipOff')"
              />
            </el-form-item>
            <el-form-item
              :label="t('simpleProcess.childProcess.inVars')"
              prop="inVariables"
            >
              <div
                class="flex pt-2"
              ...
              >
                <div class="mr-2">
                  <el-form-item
                    ...
                    :rules="{
                      required: true,
                      message: t('simpleProcess.childProcess.varRequired'),
                      trigger: 'blur'
                    }"
                  >
              ...
              >
                <Icon icon="ep:plus" class="mr-5px" />{{
                  t('simpleProcess.childProcess.addRow')
                }}
              </el-button>
            </el-form-item>
            <el-form-item
              v-if="configForm.async === false"
              :label="t('simpleProcess.childProcess.outVars')"
              prop="outVariables"
            >
              <div
                class="flex pt-2"
            ...
            <el-form-item
              :label="t('simpleProcess.childProcess.startUser')"
              prop="startUserType"
            >
            ...
            <el-form-item
              v-if="
                ...
                ChildProcessStartUserTypeEnum.FROM_FORM
              "
              :label="t('simpleProcess.childProcess.whenStartUserEmpty')"
              prop="startUserType"
            >
            ...
            <el-form-item
              ...
              :label="t('simpleProcess.childProcess.starterForm')"
              prop="startUserFormField"
            >
              <el-select
                class="w-200px!"
                v-model="configForm.startUserFormField"
              >
                <el-option
                  v-for="(field, fIdx) in formFieldOptions"
                  :key="fIdx"
            ...
            <el-divider content-position="left">{{
              t('simpleProcess.childProcess.timeoutSection')
            }}</el-divider>
            <el-form-item
              :label="t('simpleProcess.childProcess.enableSwitch')"
              prop="timeoutEnable"
            >
              <el-switch
            ...
            <div v-if="configForm.timeoutEnable">
              ...
              <el-form-item
                v-if="
                  configForm.timeoutType === DelayTypeEnum.FIXED_TIME_DURATION
                "
              >
                ...
                <el-text>{{
                  t('simpleProcess.childProcess.nextNodeAfter')
                }}</el-text>
              </el-form-item>
              <el-form-item
              ...
              >
                <el-date-picker
                  ...
                  type="datetime"
                  :placeholder="t('simpleProcess.childProcess.pickDateTime')"
            ...
            <el-divider content-position="left">{{
              t('simpleProcess.childProcess.multiInstanceSection')
            }}</el-divider>
            <el-form-item
              :label="t('simpleProcess.childProcess.enableSwitch')"
              prop="multiInstanceEnable"
            >
              <el-switch
            ...
            <div v-if="configForm.multiInstanceEnable">
              <el-form-item prop="sequential">
                <el-switch
                  ...
                  :active-text="t('simpleProcess.childProcess.serial')"
                  :inactive-text="t('simpleProcess.childProcess.parallel')"
                />
              </el-form-item>
              <el-form-item prop="approveRatio">
                <el-text>{{
                  t('simpleProcess.childProcess.approveRatio')
                }}</el-text>
                <el-input-number
                  class="ml-10px"
              ...
              <el-form-item prop="multiInstanceSourceType">
                <el-text>{{
                  t('simpleProcess.childProcess.multiInstanceSource')
                }}</el-text>
                <el-select
...
<script setup lang="ts">
  ...
  const formRules = computed(() => ({
    async: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleAsync'),
        trigger: 'change'
      }
    ],
    calledProcessDefinitionKey: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleChildProcess'),
        trigger: 'change'
      }
    ],
    skipStartUserNode: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleSkipStart'),
        trigger: 'change'
      }
    ],
    startUserType: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleStartUser'),
        trigger: 'change'
      }
    ],
    startUserEmptyType: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleStartUserEmpty'),
        trigger: 'change'
      }
    ],
    startUserFormField: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleStarterForm'),
        trigger: 'change'
      }
    ],
    timeoutEnable: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleTimeoutEnable'),
        trigger: 'change'
      }
    ],
    timeoutType: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleTimeoutType'),
        trigger: 'change'
      }
    ],
    timeDuration: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleTimeDuration'),
        trigger: 'change'
      }
    ],
    dateTime: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleDateTime'),
        trigger: 'change'
      }
    ],
    multiInstanceEnable: [
      {
        required: true,
        message: t('simpleProcess.childProcess.ruleMultiInstance'),
        trigger: 'change'
      }
    ]
  }))
  ...
  const saveConfig = async () => {
    ...
    const childInfo = childProcessOptions.value.find(
      (option: any) =>
        option.key === configForm.value.calledProcessDefinitionKey
    )
    ...
    if (currentNode.value.childProcessSetting) {
      ...
      if (configForm.value.timeoutEnable) {
        ...
        if (
          configForm.value.timeoutType === DelayTypeEnum.FIXED_TIME_DURATION
        ) {
    ...
    currentNode.value.showText = t(
      'simpleProcess.childProcess.callChildProcess',
      { name: childInfo.name }
    )
