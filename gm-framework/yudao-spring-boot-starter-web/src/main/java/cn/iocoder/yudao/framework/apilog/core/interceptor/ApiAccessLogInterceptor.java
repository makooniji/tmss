...
public class ApiAccessLogInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        // 1. 获取 traceId（优先从请求头拿）
        String traceId = request.getHeader(HEADER_TRACE_ID);

        // 2️⃣ 支持 URL 参数（如 ?traceId=xxx）
        if (StrUtil.isBlank(traceId)) {
            traceId = request.getParameter(TRACE_ID);
        }

        String requestBody = ServletUtils.isJsonRequest(request) ? ServletUtils.getBody(request) : null;
        // 2. 如果是 JSON Body（你这个接口场景）
        if (StrUtil.isBlank(traceId) && ServletUtils.isJsonRequest(request)) {

            if (StrUtil.isNotBlank(requestBody) && requestBody.contains("traceId")) {
                // 简单提取（推荐用 JSON 解析更安全）
                traceId = JSONObject.parseObject(requestBody).getString(TRACE_ID);
            }
        }
        ...
        if (!SpringUtils.isProd()) {
            ...
            if (CollUtil.isEmpty(queryString) && StrUtil.isEmpty(requestBody)) {
                log.info("[preHandle][traceId={}][开始请求 URL({}) 无参数]",traceId, request.getRequestURI());
            } else {
                log.info("[preHandle][traceId={}][开始请求 URL({}) 参数({})]",traceId, request.getRequestURI(),
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        if (!SpringUtils.isProd()) {
            ...
            stopWatch.stop();
            log.info("[afterCompletion][traceId={}][完成请求 URL({}) 耗时({} ms)]",
                    MDC.get(TRACE_ID),  request.getRequestURI(), stopWatch.getTotalTimeMillis());
        }
        // ❗必须清理，否则线程复用会串 traceId
        MDC.remove(TRACE_ID);
    }
